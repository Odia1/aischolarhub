...
def derive_name(email: str) -> str:
    """No 'name' column in this CSV format — derive a display name from the
    email local-part so create-user has something reasonable to show."""
    local = email.split("@", 1)[0]
    return local.replace(".", " ").replace("_", " ").title()


def ensure_user(container: str, email: str, name: str, temp_password: str, role: str) -> tuple[bool, str]:
    ...
    #result = docker_exec(container, "node", "/app/config/create-user", "--",                          email, name, temp_password)
    if result.returncode == 0:
        return True, "created"
    # Non-zero can mean "already exists" (common re-run case) OR a real
    # failure — surface stderr so it's not silently swallowed either way.
...
def set_balance(container: str, email: str, amount: int) -> tuple[bool, str]:
    result = docker_exec(
        container,
        "node",
        "/app/config/set-balance.js",
        email,
        str(amount),
    )
    #result = docker_exec(container, "node", "/app/config/set-balance", "--",                          email, str(amount))
    if result.returncode == 0:
        return True, ""
    return False, (result.stderr.strip() or result.stdout.strip() or "unknown error")
...
def main():
    ...
    with open(args.csv_path, newline="") as f:
        for row in csv.DictReader(f):
            ...
            temp_password = row["temp_password"].strip()
            ...
            ok, detail = ensure_user(args.container, email, name, temp_password, role)
            if not ok:
