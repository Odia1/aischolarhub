    ...
    try:
        ...
        if body.file_id in rag_ids:
            ...
            raise HTTPException(
                ...
                detail="The specified file_id was not found",
            )
...
async def embed_file(
    ...
    user_id = get_user_id(request, entity_id)
    tenant_id = (
        request.state.user.get("tenantId")
...
async def query_embeddings_by_file_ids(request: Request, body: QueryMultipleBody):
    ...
    try:
        ...
        if not branches:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="No authorized file_ids were found",
            )

        if len(branches) == 1:
            authorization_clause = branches[0]
        else:
            authorization_clause = {"$or": branches}
        ...
        # Perform similarity search with the query embedding and filter by the authorized file_ids
        if isinstance(vector_store, AsyncPgVector):
            documents = await vector_store.asimilarity_search_with_score_by_vector(
                embedding,
                k=body.k,
