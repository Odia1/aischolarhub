...
(() => {
  const boot = () => {
    ...
    const state = { departments: [], courses: [], groups: [], rag: [], users: [] };
    ...
    window.openGroupForm = function(existing=null){
      const selectedMembers=existing?.memberIds||[], selectedDepartments=existing?.departmentIds||[], selectedCourses=existing?.courseIds||[];
      dialog(existing?'Edit Group':'Create Group',
        `<input name="name" required maxlength="200" placeholder="Group name" value="${esc2(existing?.name||'')}">
         <textarea name="description" maxlength="1000" placeholder="Description (optional)">${esc2(existing?.description||'')}</textarea>
         <label class="muted">Members (users/instructors)</label><select name="memberIds" multiple>${options(state.users,selectedMembers)}</select>
         <label class="muted">Departments / Schools</label><select name="departmentIds" multiple>${options(state.departments,selectedDepartments)}</select>
         <label class="muted">Courses / Classes</label><select name="courseIds" multiple>${options(state.courses,selectedCourses)}</select>`,
        async d=>{const f=d.querySelector('form');const vals=n=>[...f[n].selectedOptions].map(o=>o.value);await api(existing?`/api/groups/${encodeURIComponent(id(existing))}`:'/api/groups',{method:existing?'PATCH':'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:f.name.value,description:f.description.value,memberIds:vals('memberIds'),departmentIds:vals('departmentIds'),courseIds:vals('courseIds')})})});
    };
    ...
    function render(){
      ...
      renderList(document.getElementById('groupList'),state.groups,g=>`<div class="org-row"><div><strong>${esc2(g.name)}</strong><div class="org-small">${(g.memberIds||[]).length} members · ${(g.departmentIds||[]).length} departments · ${(g.courseIds||[]).length} courses</div></div><div class="org-actions"><button type="button" onclick="openEncoded('group','${arg(g)}')">Edit</button><button type="button" class="danger" onclick="deleteEncoded('group','${arg(g)}')">Delete</button></div></div>`);
    ...
    window.loadOrganizationAdmin = async function(){
      try{
        ...
        const [d,c,g,r,u]=await Promise.all([api(`/api/departments${q}`),api(`/api/courses${q}`),api(`/api/groups${q}`),api(`/api/rag-locations${q}`),api('/api/users?q=&role=')]);
        state.departments=d.departments||[];state.courses=c.courses||[];state.groups=g.groups||[];state.rag=(r.locations||[]).filter(x=>!x.automatic);state.users=Array.isArray(u)?u:[];
